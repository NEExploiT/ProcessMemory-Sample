...
print(
  CSV.generate('', encoding: 'cp932') do |csv|
    # 通常変数
    (14..22).each.with_index do |t, it|
      csv << ["# 変数 v#{it}"]
      sdb(t).keys.each.with_index do |name, ix|
    ...
    csv << ['# 文字変数 s']
    svkeys = sdb(4).keys
    WditStringArray.new(SVAR_BASE).to_a.each.with_index do |s, ix|
      csv << ["s[#{ix}]", svkeys[ix], s]
    end
    # 可変データベース
    vdb.keys.each.with_index do |tname, it|
      db_t = vdb(it)
      csv << ["# vdb[#{it}]:#{tname}"]
      db_t.keys.each.with_index do |dname, id|
        db_td = db_t[id]
        row = ["vdb[#{it},#{id}]:#{dname}"]
        db_td.keys.each.with_index do |cname, ic|
          db = db_td[ic]
